...
contract UserVault is ERC721Holder, ERC1155Holder, ReentrancyGuard {
    ...
    function buyFraction(uint256 _amount) external nonReentrant {
        ...
        uint256 amount = _amount * ommi_price;
        require(
            OmmiToken.balanceOf(msg.sender) >= amount,
            "Not enough balance"
        );
        uint256 feeAmount = amount * ((ISettings(settings).fee()) / 1000);
        uint256 afterFee = amount - feeAmount;
        OmmiToken.safeTransferFrom(msg.sender, curator, afterFee);
        _sendOmmi(ISettings(settings).feeReceiver(), feeAmount);
